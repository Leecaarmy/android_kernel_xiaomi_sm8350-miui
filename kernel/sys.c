...
SYSCALL_DEFINE1(newuname, struct new_utsname __user *, name)
{
	 ...
	 * required 5.10 BPF backport, so expose the compatibility level only
	 * to that loader. Keep the real release visible to every other caller.
	 */
	if (!strcmp(current->comm, "netbpfload"))
		strscpy(tmp.release, "5.10.199-dsu-bpf-compat", sizeof(tmp.release));
	up_read(&uts_sem);
