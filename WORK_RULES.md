...
- 只允许修改内核源码。
- 严禁对未获用户明确授权的任何设备分区执行修改、写入、擦除、格式化或切换操作。对用户明确授权的目标 boot，只允许执行本手册第 6 节规定的 kernel-only 替换。
- 严禁修改或注入 `ramdisk/`、`patch/`、`modules/` 及其对应启动镜像中除 kernel payload 外的内容；kernel-only 重封装不得改变 ramdisk cpio、启动配置、DTB/DTBO 或其他组件。
...
- 若工具或刷机包会修改 ramdisk、启动配置、模块、AVB/vbmeta 或未获授权的其他分区，应停止并说明，不得继续执行。经用户明确授权的目标 boot，可以按照第 6 节使用 `split_boot` + `flash_boot` 重新封装，仅替换 kernel payload；这种必要的 boot 重封装不属于修改 ramdisk、启动配置或其他组件。
- 任何超出本规则范围的操作，都必须先取得用户明确授权；打包和离线验证不得写入手机分区。
