...
本轮 AK3 入口通过 shell 语法、ZIP 完整性和本地脚本路径检查。没有为了测试包而在手机上永久刷写，因此不宣称 recovery 永久安装已经实测。

Horizon 安装准入通过离线测试：模拟 zygote 存在和锁定状态属性时，四个允许标识均通过唯一的机型检查，其他机型仍拒绝。Horizon 的实际分区写入和刷后重启未实机验证，不能将入口兼容或本地脚本测试等同于已完成刷机验证。
